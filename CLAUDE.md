...
**Node Version**: 22.21.1 (pinned in .nvmrc)
...
docker logs raycast-ai-litellm-proxy-raycast-ai-proxy-1  # Check logs
```

### Critical: Docker Networking

When running in Docker, `localhost` inside the container refers to the container itself, NOT your host machine.

| Context | URL to reach host's LiteLLM |
|---------|----------------------------|
| Docker container | `http://host.docker.internal:4000/v1` |
| Local dev (no Docker) | `http://localhost:4000/v1` |

**Common mistake**: Using `localhost` in `.env` when running via Docker → connection refused.

The error handler will warn you:
```
Cannot connect to LiteLLM at http://localhost:4000/v1.
If running in Docker, use host.docker.internal instead of localhost.
```

### Shadow Process Gotcha

If you're testing changes but seeing old behavior, check if Docker is running an old version:
```bash
lsof -i :11435  # Should show node, not com.docker
...
**Zod Validation (v4 API)**:
- Import from `zod/v4` subpath for strict validation: `import { z } from 'zod/v4'`
- Zod v4 rejects unknown fields by default - use `.passthrough()` to allow extra fields
- For nullable fields: use `.nullable().optional()` (not just `.optional()`)
- Use `.passthrough()` on schemas that parse external API responses (allows forward compatibility)
- Example: `z.object({ field: z.string() }).passthrough()` allows extra fields beyond `field`

**Null Handling**:
- Use nullish coalescing `??` for optional/nullable values: `value ?? fallbackValue`
- Avoid `||` operator when falsy values (0, false, '') are valid
<!-- END AUTO-MANAGED -->
